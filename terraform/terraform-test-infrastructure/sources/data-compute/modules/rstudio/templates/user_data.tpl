...
echo "mount_target_a=${RstudioMountTargetA}" > /etc/facter/facts.d/mount_target_a.txt
echo "mount_target_b=${RstudioMountTargetB}" > /etc/facter/facts.d/mount_target_b.txt
echo "ec2_region=${region}" > /etc/facter/facts.d/ec2_region.txt

cat > /etc/ecs/ecs.config << EOL
  ECS_CLUSTER=${RstudioEcsCluster}
...
echo "ecs_cluster=${RstudioEcsCluster}" > /etc/facter/facts.d/ecs_cluster.txt
...
cat > /etc/puppet/site.pp << EOL
    ...
    cloudwatchlogs::log { 'cloud-init.log':
      ...
      log_group_name  => '/aws/ec2/rstudio',
