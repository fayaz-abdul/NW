resource "aws_efs_file_system" "logstash_mounts" {
  throughput_mode = "bursting"
}

resource "aws_efs_mount_target" "mount_a" {
  file_system_id = "${aws_efs_file_system.logstash_mounts.id}"
...
resource "aws_security_group" "efs_secgrp" {
  name        = "${join("-", compact(list(var.environment, var.customer, var.project, var.role, var.application, "logstash", "efs", "secgroup")))}"
  description = "Control security for Logstash EFS"
  vpc_id      = "${var.vpc_id}"
  ...
  tags = "${merge(
    ...
    zipmap(
      ...
      compact(list(
        join("-", compact(list(var.environment, var.customer, var.project, var.role, var.application, "logstash", "efs", "secgroup"))),
        var.environment,
        var.customer,
        var.project,
        var.role,
